...
typedef enum {CMenuEx_DRAWMODE_ORIGINAL,CMenuEx_DRAWMODE_XP} BC_MenuDrawMode;

// how seperators are handled when removing a menu (Tongzhe Cui)
typedef enum {CMenuEx_NONE, CMenuEx_HEAD, CMenuEx_TAIL, CMenuEx_BOTH} BC_Seperator;
...
public:
	...
	CMenuEx* AppendODPopupMenuA(LPCSTR lpstrText);
	CMenuEx* AppendODPopupMenuW(wchar_t *lpstrText);
	...
	int RemoveMenu(char* pText, BC_Seperator sPos=CMenuEx_NONE);
	int RemoveMenu(wchar_t* pText, BC_Seperator sPos=CMenuEx_NONE);
	int DeleteMenu(char* pText, BC_Seperator sPos=CMenuEx_NONE);
	int DeleteMenu(wchar_t* pText, BC_Seperator sPos=CMenuEx_NONE);
	...
	CMenuEx* GetSubCMenuEx(char* lpszSubMenuName);
	CMenuEx* GetSubCMenuEx(wchar_t* lpszSubMenuName);
	...
	static void SetMenuDrawMode(UINT mode){
		CMenuEx::original_drawmode=mode;
		CMenuEx::xp_drawmode=mode;
	};
	...
	static void SetSelectDisableMode(BOOL mode){
		CMenuEx::original_select_disabled=mode;
		CMenuEx::xp_select_disabled=mode;
	};

	static int CMenuEx::GetMenuDrawMode(void);
	static BOOL CMenuEx::GetSelectDisableMode(void);

	// how the bitmaps are drawn in XP Luna mode
	static void SetXPBitmap3D(BOOL val){
		CMenuEx::xp_draw_3D_bitmaps=val;
	};
	static BOOL GetXPBitmap3D(void){return CMenuEx::xp_draw_3D_bitmaps;}
...
#define CMenuEx_USE_MEMDC

#ifdef CMenuEx_USE_MEMDC
//////////////////////////////////////////////////
// CMenuExMemDC - memory DC
//
// Author: Keith Rule
