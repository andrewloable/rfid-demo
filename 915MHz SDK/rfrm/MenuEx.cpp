...
void CMenuEx::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(state & MF_SEPARATOR){
		...
		rect.left = rect.left + m_iconX + 8 + CMenuEx_GAP; 
...
COLORREF CMenuEx::LightenColor(COLORREF col,double factor)
{
...
COLORREF CMenuEx::DarkenColor(COLORREF col,double factor)
{
	if(factor>0.0&&factor<=1.0){
		BYTE red,green,blue,lightred,lightgreen,lightblue;
		red = GetRValue(col);
...
void CMenuEx::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
#ifdef CMenuEx_USE_MEMDC
	CMenuExMemDC *pMemDC=NULL;
#endif
	CRect rect,rect2;
	UINT state = (((CMenuExData*)(lpDIS->itemData))->nFlags);
	...
	int CMenuEx_PAD=4;
	if(xp_draw_3D_bitmaps)CMenuEx_PAD=7;
	int barwidth=m_iconX+CMenuEx_PAD;
	
	// remove the selected bit if it's grayed out
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled){
		if(lpDIS->itemState & ODS_SELECTED)lpDIS->itemState=lpDIS->itemState & ~ODS_SELECTED;
#ifdef CMenuEx_USE_MEMDC
		pMemDC=new CMenuExMemDC(pDC,&lpDIS->rcItem);
		pDC = pMemDC;
	...
	if(state & MF_SEPARATOR){
		...
		rect.top+=rect.Height()>>1;
		rect.left = rect2.right+CMenuEx_PAD;
		pDC->DrawEdge(&rect,EDGE_ETCHED,BF_TOP);
...
BOOL CMenuEx::ModifyODMenuA(const char * lpstrText,UINT nID,CBitmap *bmp)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,bmp);
}

BOOL CMenuEx::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
...
BOOL CMenuEx::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,fill,border,hatchstyle,pSize);
}

BOOL CMenuEx::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
...
// O.S.
CMenuExData *CMenuEx::FindMenuItem(UINT nID)
{
	CMenuExData *pData = NULL;
	int i;
	...
	if (!pData){
		int loc;
		CMenuEx *pMenu = FindMenuOption(nID, loc);
		ASSERT(pMenu != this);
		if (loc >= 0){
...
CMenuEx *CMenuEx::FindAnotherMenuOption(int nId,int& nLoc,CArray<CMenuEx*,CMenuEx*>&bcsubs,
									  CArray<int,int&>&bclocs)
{
	int i,numsubs,j;
...
#ifdef _CPPRTTI 
		psubmenu=dynamic_cast<CMenuEx *>(GetSubMenu(i));
#else
		psubmenu=(CMenuEx *)GetSubMenu(i);
#endif
		if(psubmenu){
...
BOOL CMenuEx::LoadMenu(HINSTANCE hInstance, int nResource)
{
	return(CMenuEx::LoadMenu(hInstance, MAKEINTRESOURCE(nResource)));
}
...
BOOL CMenuEx::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	if (NULL != hInst)
	{
		return CMenuEx::LoadMenu(hInst, lpszResourceName);
	}
	return FALSE;
}


BOOL CMenuEx::LoadMenu(HINSTANCE hInst, LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	ASSERT(lpszResourceName != NULL);
...
void CMenuEx::RemoveTopLevelOwnerDraw(void)
{
	CString str;
	int i=0,j=0;
...
//--------------------------------------------------------------------------
//[18.06.99 rj]
BOOL CMenuEx::GetMenuText(UINT id, CString& string, UINT nFlags/*= MF_BYPOSITION*/)
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags){
		...
		int uiLoc;
		CMenuEx* pMenu = FindMenuOption(id,uiLoc);
...
void CMenuEx::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color,BOOL narrowflag)
{
	int dp=0;
	CPen m_penBack;
                                     ...
                                     CMenu *pMenu)
{
	...
	if(pCMenuEx && nFlags&MF_POPUP){
		...
		key.MakeLower();
		CString menutext;
		int menusize = (int)pCMenuEx->GetMenuItemCount();
...
void CMenuEx::GetFadedBitmap(CBitmap &bmp)
{
	CDC ddc;
	COLORREF bgcol,col;
	BITMAP BitMap;
...
void CMenuEx::GetTransparentBitmap(CBitmap &bmp)
{
	CDC ddc;
...
void CMenuEx::GetDisabledBitmap(CBitmap &bmp,COLORREF background)
{
	CDC ddc;
...
void CMenuEx::GetShadowBitmap(CBitmap &bmp)
{
	CDC ddc;
...
BOOL CMenuEx::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	BOOL bReturn=FALSE;
...
BOOL CMenuEx::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout,BOOL selected)
{
	CBrush brushin;
	COLORREF col;
	int dx,dy;
...
void CMenuEx::DitherBlt3(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
WORD CMenuEx::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
BOOL CMenuEx::RemoveMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			CMenuEx* pSubMenu = (CMenuEx*) GetSubMenu(uiId);
			if(NULL==pSubMenu){
		...
		int iPosition =0;
		CMenuEx* pMenu = FindMenuOption(uiId,iPosition);
...
BOOL CMenuEx::DeleteMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		UINT uint = GetMenuState(uiId,MF_BYPOSITION);
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
...
BOOL CMenuEx::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CMenuEx::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CMenuEx::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CMenuEx::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CMenuEx::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CMenuEx::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
BOOL CMenuEx::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CMenuEx::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CMenuEx::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CMenuEx::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CMenuEx::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CMenuEx::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CMenuEx* CMenuEx::AppendODPopupMenuW(wchar_t *lpstrText)
{
	CMenuEx* pSubMenu = new CMenuEx;
	pSubMenu->m_unselectcheck=m_unselectcheck;
	pSubMenu->m_selectcheck=m_selectcheck;
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CMenuEx* CMenuEx::AppendODPopupMenuA(LPCSTR lpstrText)
{
	USES_CONVERSION;
	return AppendODPopupMenuW(A2W(lpstrText));
}

BOOL CMenuEx::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
...
CMenu* CMenuEx::GetSubMenu(int nPos)
{
	return CMenu::GetSubMenu (nPos);
}

CMenu* CMenuEx::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
CMenuEx* CMenuEx::GetSubCMenuEx(char* pText)
{
	USES_CONVERSION;
	return GetSubCMenuEx(A2W(pText));
}

CMenuEx* CMenuEx::GetSubCMenuEx(wchar_t* lpszSubMenuName)
{
	CMenuExData *mdata;
...
int CMenuEx::GetMenuPosition(char* pText)
{
	USES_CONVERSION;
	return GetMenuPosition(A2W(pText));
}

int CMenuEx::GetMenuPosition(wchar_t* pText)
{
	int i,j;
	CMenuEx* psubmenu;
	for(i=0;i<(int)(GetMenuItemCount());++i)
...
int CMenuEx::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int CMenuEx::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
	...
	if(sPos == CMenuEx_HEAD)
	{
		ASSERT(nPos - 1 >= 0);
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMenuEx_TAIL)
	{
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMenuEx_BOTH)
	{
...
int CMenuEx::DeleteMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return DeleteMenu(A2W(pText), sPos);
}

int CMenuEx::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
	int nPos = GetMenuPosition(pText);
	if(nPos != -1)
	{
...
// Larry Antram
BOOL CMenuEx::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
...
// courtesy of Warren Stevens
void CMenuEx::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	CDC bmpdc;
	COLORREF m_newclrBack;
...
BOOL CMenuEx::IsWindowsClassicTheme(void)
{
...
int CMenuEx::GlobalImageListOffset(int nID)
{
...
BOOL CMenuEx::CanDraw3DImageList(int offset)
{
	BOOL retflag=FALSE;
	INT_PTR numcurrent=m_AllImagesID.GetSize();
...
int CMenuEx::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	int loc = -1;
