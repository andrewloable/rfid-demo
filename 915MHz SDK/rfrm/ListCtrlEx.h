...
#ifndef __LISTCTRL_H__
#define __LISTCTRL_H__
...
{
class CHeaderCtrlEx;
// CListCtrlEx is a CListCtrl derived class which implements a
...
inline DWORD CListCtrlEx::GetExtendedStyle()
	{ ASSERT(::IsWindow(m_hWnd)); return (DWORD)SendMessage(LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0); }

inline void CListCtrlEx::SetExtendedStyle(DWORD dwExStyle)
	{ ASSERT(::IsWindow(m_hWnd)); SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0, GetExtendedStyle()|dwExStyle); }

inline void CListCtrlEx::SetRowColors(COLORREF clrText, COLORREF clrTextBk)
	{ m_clrText = clrText; m_clrTextBk = clrTextBk; }

inline void CListCtrlEx::SetMinimumColSize(int nSize)
	{ m_nMinSize = nSize; }

inline void CListCtrlEx::DisableColumnSizing(bool bDisable)
